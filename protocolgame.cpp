...
void ProtocolGame::spectate(const std::string& name, const std::string& password)
{
	...
	{
		disconnectClient(0x14, "Transmissão invalida.");
		return;
	}
	...
	OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster)
{
		...
		(accountManager && name == "Account Manager"))
	{
		...
		{
			disconnectClient(0x14, "Não foi possível carregar seu personagem.");
			return;
		}
		...
		{
			disconnectClient(0x14, "Você não é um gamemaster! Desligue o modo gamemaster no seu trocador de IP.");
			return;
		}
		...
		if(!player->hasFlag(PlayerFlag_CanAlwaysLogin))
		{
			...
			{
				disconnectClient(0x14, "O jogo está desligado.\nPor favor, tente novamente mais tarde.");
				return;
			}

			if(g_game.getGameState() == GAMESTATE_CLOSED)
			{
				disconnectClient(0x14, "O servidor está fechado no momento.\nPor favor, tente novamente mais tarde.");
				return;
			}
		}
			...
			!IOLoginData::getInstance()->hasCustomFlag(id, PlayerCustomFlag_CanLoginMultipleCharacters))
		{
			...
			if(tmp.size() > 0 && !found)
			{
				disconnectClient(0x14, "Você pode fazer login apenas com um jogador\n na sua conta ao mesmo tempo.");
				return;
			}
		}
		...
		{
			disconnectClient(0x14, "A posição do templo está errada.");
			return;
		}
...
bool ProtocolGame::logout(bool displayEffect, bool forceLogout)
{
	...
	{
		player->sendTextMessage(MSG_STATUS_SMALL, "Você tem que esperar um pouco.");
		return false;
	}
	...
	if(!player->isRemoved())
	{
		...
		if(displayEffect && !player->isGhost())
			g_game.addMagicEffect(player->getPosition(), 5);
	}
...
bool ProtocolGame::connect(uint32_t playerId, OperatingSystem_t operatingSystem, uint16_t version)
{
	...
	{
		disconnectClient(0x14, "Voc? j? est? logado.");
		return false;
	}
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	sendExtendedOpcode(0x32, std::string());
	sendExtendedOpcode(0x00, std::string());
    sendFeatures();//Aion OTCV8
    otclientV8 = 265;//Aion OTCV8
...
if(!IOLoginData::getInstance()->playerExists(character))
    {
        disconnectClient(0x14, "Este personagem não existe.");
        return;
    }
	msg.skipBytes(6);
	...
	{
		disconnectClient(0x14, "Gameworld está iniciando. Por favor, espere.");
		return;
	}

	if(g_game.getGameState() == GAMESTATE_MAINTAIN)
	{
		disconnectClient(0x14, "O Gameworld está em manutenção. Reconecte-se daqui a pouco.");
		return;
	}

	if(IOBan::getInstance()->isIpBanished(getIP()))
	{
		disconnectClient(0x14, "Seu IP foi banido.");
		return;
	}
	...
	{
		disconnectClient(0x14, "O nome ou a senha da conta não estão corretos.");
		return;
	}
	...
	if(IOBan::getInstance()->getData(ban) && !IOLoginData::getInstance()->hasFlag(id, PlayerFlag_CannotBeBanned))
	{
		...
		std::stringstream stream;
		stream << "Sua conta foi " << (deletion ? "deletada" : "banida") << " até:\n" << formatDateEx(ban.added, "%d %b %Y").c_str() << " por: " << name_.c_str()
			   << ".\nO comentário feito foi:\n" << ban.comment.c_str() << ".\nSua " << (deletion ? "conta não será recuperada" : "o banimento será suspenso:\n")
			   << (deletion ? "" : formatDateEx(ban.expires).c_str()) << ".";
...
{
	return !c->isRemoved() && player->canSeeCreature(c) && canSee(c->getPosition()); // changed by luan
}
...
void ProtocolGame::parseSetOutfit(NetworkMessage& msg)
{
    ...
    if (g_config.getBool(ConfigManager::ALLOW_CHANGEOUTFIT)) {
        uint16_t requestedLookType = msg.get<uint16_t>();

        std::vector<uint16_t> restrictedLookTypes = {2317, 2307, 1788, 22, 1743, 1744, 2134, 1745, 1746, 1881, 2212, 2198, 2196, 2195, 2197, 2199, 1915, 1226, 1225, 2213, 1992, 1527, 1540, 1543, 1534, 1526, 1537, 1533, 1512, 1509, 1516, 1529, 1517, 1514, 1515, 1546, 1525, 2286, 1508, 2295, 2293, 2278, 1547, 1538, 2582, 1513, 1510, 1317, 1524, 1542, 1530, 1528, 1536, 1548, 1511, 1544, 2175, 2207, 2181, 1511};
        
        if (std::find(restrictedLookTypes.begin(), restrictedLookTypes.end(), requestedLookType) != restrictedLookTypes.end()) {
            player->sendTextMessage(MSG_STATUS_SMALL, "Você não pode mudar para essa saga.");
            return;
        }
        
        std::unordered_map<uint16_t, std::vector<uint16_t>> allowedOutfits = {
            {1792, {10}},	// Tobirama
			{1793, {14}},	// Obito
			{1794, {12}},	// Minato
			{2297, {2}},	// Sasuke
			...
			{2298, {31}},	// Shisui
			{1755, {13}},	// Itachi
			{2321, {10}},	// Tobirama shodaime
			{2322, {10}},	// Tobirama shodaime
        };

        if (allowedOutfits.find(requestedLookType) != allowedOutfits.end()) {
            const std::vector<uint16_t>& allowedVocations = allowedOutfits[requestedLookType];
            uint16_t playerVocationId = player->getVocationId();
            if (std::find(allowedVocations.begin(), allowedVocations.end(), playerVocationId) == allowedVocations.end()) {
                player->sendTextMessage(MSG_STATUS_SMALL, "Seu personagem não pode utilizar essa skin.");
                return;
            }
        }

        const Outfit_t& currentOutfit = player->getCurrentOutfit();
        if (std::find(restrictedLookTypes.begin(), restrictedLookTypes.end(), currentOutfit.lookType) != restrictedLookTypes.end()) {
            player->sendTextMessage(MSG_STATUS_SMALL, "Você não pode mudar de saga.");
            return;
        }

        newOutfit.lookType = requestedLookType;
    } else {
        msg.skipBytes(2);
    }
    ...
    newOutfit.lookAura = player->getCurrentOutfit().lookAura;
    newOutfit.lookWings = player->getCurrentOutfit().lookWings;
    newOutfit.healthBackground = player->getCurrentOutfit().healthBackground;
    newOutfit.manaBackground = player->getCurrentOutfit().manaBackground;

    addGameTask(&Game::playerChangeOutfit, player->getID(), newOutfit);

    // Animated Text Display
    std::vector<uint16_t> restrictedLookTypesForTextDisplay = {1825, 1821, 1822, 1823, 1824, 2321, 2322};
    if (std::find(restrictedLookTypesForTextDisplay.begin(), restrictedLookTypesForTextDisplay.end(), newOutfit.lookType) != restrictedLookTypesForTextDisplay.end()) {
        std::string outfitText;
        int textColor = COLOR_WHITE;
        switch(newOutfit.lookType) {
            case 1825:
                outfitText = "Hokage!";
                textColor = COLOR_RED;
                break;
        ...
        g_game.addAnimatedText(player->getPosition(), textColor, outfitText);
    }
        ...
        query << "INSERT INTO `player_newoutfit` (`player_id`, `looktype`) VALUES (" << player->getGUID() << ", " << newOutfit.lookType <<")";
...
void ProtocolGame::sendContainer(uint32_t cid, const Container* container, bool hasParent)
{
	...
	msg->addItem(container, otclientV8 != 0);//AION OTCV8 TOOL
	...
	for(uint32_t i = 0; cit != container->getEnd() && i < 255; ++cit, ++i)
		msg->addItem(*cit, otclientV8 != 0);//AION OTCV8 TOOL
}
...
void ProtocolGame::sendCreatureJump(uint32_t creatureId, uint32_t height, uint32_t duration, bool straight)
{
	OutputMessage_ptr msg = getOutputBuffer();
	if(!msg)
		return;
	...
	msg->add<uint32_t>(height);
	msg->add<uint32_t>(duration);
	msg->addByte(straight ? 0x01 : 0x00);
}
...
void ProtocolGame::sendTradeItemRequest(const Player* _player, const Item* item, bool ack)
{
	...
	if(const Container* container = item->getContainer())
	{
		...
		msg->addItem(item, otclientV8 != 0);//AION OTCV8 TOOL
		for(ContainerIterator it = container->begin(); it != container->end(); ++it)
			msg->addItem(*it, otclientV8 != 0);//AION OTCV8 TOOL
	}
	else
	{
		msg->addByte(1);
...
void ProtocolGame::sendOutfitWindow()
{
	...
	// Thalles Vitor
	std::vector<int> listWings;
	listWings.push_back(player->getDefaultOutfit().lookWings);

	for (size_t i = 0; i <= listWings.size(); i++) {
		if (listWings[i]) {
			msg->addByte(i);
			msg->add<uint16_t>(listWings[i]);
			msg->addString("Your wings");
			msg->addByte(0);
		}
		msg->addByte(0);
	}

	std::vector<int> listAura;
	listAura.push_back(player->getDefaultOutfit().lookAura);

	for (size_t i = 0; i <= listAura.size(); i++) {
		if (listAura[i]) {
			msg->addByte(i);
			msg->add<uint16_t>(listAura[i]);
			msg->addString("Your aura");
			msg->addByte(0);
		}
		msg->addByte(0);
	}
...
void ProtocolGame::AddTileItem(OutputMessage_ptr msg, const Position& pos, uint32_t stackpos, const Item* item)
{
	...
	msg->addPosition(pos);
	msg->addByte(stackpos);
	msg->addItem(item, false);//AION OTCV8 TOOL
}
...
void ProtocolGame::UpdateTileItem(OutputMessage_ptr msg, const Position& pos, uint32_t stackpos, const Item* item)
{
	...
	msg->addPosition(pos);
	msg->addByte(stackpos);
	msg->addItem(item, false);//AION TOOL
}
...
void ProtocolGame::sendFeatures()  //Aion OTCV8
{
	if (!otclientV8)
		return;
