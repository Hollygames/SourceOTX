...
*/
    ...
    getSpectators(spectators, creature->getPosition(), false, true);
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	player->setNextAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::ACTIONS_DELAY_INTERVAL) - 0);
	return true;
}
...
bool Game::playerRequestAddVip(uint32_t playerId, const std::string& vipName)
{
	...
	player->setNextExAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::CUSTOM_ACTIONS_DELAY_INTERVAL) - 0);
...
bool Game::playerPassPartyLeadership(uint32_t playerId, uint32_t newLeaderId)
{
	...
	if(player->hasCondition(CONDITION_HASTE, 27))
	{
	player->sendTextMessage(MSG_STATUS_SMALL, "You cannot do this in a quest area.");
	return false;
	}
