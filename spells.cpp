...
bool Spell::checkSpell(Player* player) const
{
	...
	if (!player->hasFlag(PlayerFlag_HasNoExhaustion))
	{
		...
		if (player->hasCondition(CONDITION_EXHAUST, EXHAUST_SPELLGROUP_SPECIAL2) && (exhaustedGroup == "special2" || exhaustedGroup == "special2"))
		{
			player->sendCancelMessage(RET_YOUAREEXHAUSTED);
			if (isInstant())
				player->sendMagicEffect(player->getPosition(), MAGIC_EFFECT_POFF);
		...
		if (player->hasCondition(CONDITION_EXHAUST, EXHAUST_SPELLGROUP_SPECIAL3) && (exhaustedGroup == "special3" || exhaustedGroup == "special3"))
		{
			player->sendCancelMessage(RET_YOUAREEXHAUSTED);
			if (isInstant())
				player->sendMagicEffect(player->getPosition(), MAGIC_EFFECT_POFF);
		...
		if (player->hasCondition(CONDITION_EXHAUST, EXHAUST_SPELLGROUP_KAWARIMI) && (exhaustedGroup == "kawarimi" || exhaustedGroup == "kawarimi"))
		{
			player->sendCancelMessage(RET_YOUAREEXHAUSTED);
			if (isInstant())
				player->sendMagicEffect(player->getPosition(), MAGIC_EFFECT_POFF);
...
void Spell::postSpell(Player* player) const
{
	if (!player->hasFlag(PlayerFlag_HasNoExhaustion) && exhaustion > 0)
	{
		...
		else if (exhaustedGroup == "special2" || exhaustedGroup == "special2")
			player->addExhaust(exhaustion, EXHAUST_SPELLGROUP_SPECIAL2);
		else if (exhaustedGroup == "special3" || exhaustedGroup == "special3")
			player->addExhaust(exhaustion, EXHAUST_SPELLGROUP_SPECIAL3);
		else if (exhaustedGroup == "kawarimi" || exhaustedGroup == "kawarimi")
			player->addExhaust(exhaustion, EXHAUST_SPELLGROUP_KAWARIMI);
	}
...
bool InstantSpell::Levitate(const InstantSpell*, Creature* creature, const std::string& param)
{
    ...
    if (tmpParam == "up") {
		std::vector<uint16_t> possibleIDs = {4472, 459, 4468, 4469, 4470, 4471, 4473, 4474, 4475, 4476, 4477, 4478, 4479, 4480, 4481, 4482, 4483, 4484, 4485, 4486, 4487, 4488, 4489, 4490, 4491, 4492, 4493, 4494, 4495, 4496, 4497, 4498, 4499, 4500, 4501, 4502, 4503, 4504, 4505, 4506, 4507, 4508, 4509, 4510, 4511, 4512, 4513, 878, 903, 904, 906, 907, 908, 909, 910, 911, 912, 913, 11249, 11250, 11251, 11252, 11253, 11254, 11255, 11256, 11257, 11258, 11259, 11260, 11261, 11262, 11263, 11264, 11265, 11266, 11267, 11268, 11269, 11270, 11271, 11272, 11273};
