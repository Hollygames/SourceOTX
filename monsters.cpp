...
void MonsterType::reset()
{
	...
	outfit.lookHead = outfit.lookBody = outfit.lookLegs = outfit.lookFeet = outfit.lookType = outfit.lookTypeEx = outfit.lookAura = outfit.lookAddons = 0;
...
ItemList MonsterType::createLoot(const LootBlock& lootBlock)
{
    ...
    {
        uint16_t n = Item::items[item].stackable ? std::min(count, (uint16_t)100) : count;

        Item* tmpItem = Item::CreateItem(item, n);
        if (!tmpItem)
            break;
        ...
        uint8_t randomAttribute = random_range(1, 3);
        ...
        {
            std::map<int, std::string> raridade = {{1, "[N] "}, {2, "[R] "}, {3, "[L] "}};
...
{
    auto it = lootBlock.childLoot.cbegin();

    if (it == lootBlock.childLoot.cend())
        return true;

    ItemList items;

    for (; it != lootBlock.childLoot.cend() && !parent->full(); ++it)
    {
        items = createLoot(*it);
...
{
    return static_cast<uint16_t>(std::ceil(static_cast<double>(random_range(0, MAX_LOOTCHANCE)) / g_config.getDouble(ConfigManager::RATE_LOOT)));
}

void MonsterType::dropLoot(Container* corpse)
{
    for (const auto& it : lootItems)
    {
        ItemList items = createLoot(it);
        if (items.empty())
            continue;

        for (const auto& iit : items)
        {
            if (Container* container = iit->getContainer())
            {
                if (createChildLoot(container, it))
                    corpse->addItem(iit);
                else
    ...
    if (message == LOOTMSG_IGNORE)
        message = static_cast<LootMessage_t>(g_config.getNumber(ConfigManager::LOOT_MESSAGE));

    if (message < LOOTMSG_PLAYER)
        return;

    std::stringstream ss;
    ss << "Loot de " << nameDescription << ": " << corpse->getContentDescription() << ".";
    std::string messageString = ss.str();

    if (owner->getParty() && message > LOOTMSG_PLAYER)
    {
        if (messageString.find("[L]") != std::string::npos ||
            messageString.find("[R]") != std::string::npos ||
            messageString.find("[N]") != std::string::npos)
        {
            owner->getParty()->broadcastMessage(static_cast<MessageClasses>(g_config.getNumber(ConfigManager::LOOT_MESSAGE_TYPE)), messageString);
        }
    }
    else if (message == LOOTMSG_PLAYER || message == LOOTMSG_BOTH)
    {
        if (messageString.find("[L]") != std::string::npos)
            g_game.addAnimatedText(corpse->getPosition(), COLOR_DARKPURPLE, "Item lendario dropado!");
        if (messageString.find("[R]") != std::string::npos)
            g_game.addAnimatedText(corpse->getPosition(), COLOR_YELLOW, "Item raro dropado!");
        if (messageString.find("[N]") != std::string::npos)
            g_game.addAnimatedText(corpse->getPosition(), COLOR_GREEN, "Item normal dropado!");
    }

    owner->sendTextMessage(static_cast<MessageClasses>(g_config.getNumber(ConfigManager::LOOT_MESSAGE_TYPE)), messageString);
}
		...
		else if(tmpName == "outfit")
		{
			...
			if(outfits.empty())
			{
				if(readXMLInteger(node, "type", intValue))
				{
					...
					if(readXMLInteger(node, "aura", intValue))
						outfit.lookAura = intValue;
	...
	for(xmlNodePtr p = root->children; p; p = p->next)
	{
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"look"))
		{
			if(readXMLInteger(p, "type", intValue))
			{
				...
				if(readXMLInteger(p, "aura", intValue))
					mType->outfit.lookAura = intValue;
