...
void ScriptEnviroment::streamOutfit(std::stringstream& stream, const std::string& local, const Outfit_t& outfit)
{
	...
	stream << "lookAura = " << outfit.lookAura << "," << std::endl;
	stream << "lookWings = " << outfit.lookWings << "," << std::endl;
...
Outfit_t LuaInterface::popOutfit(lua_State* L)
{
	...
	outfit.lookWings = getField(L, "lookWings");
...
void LuaInterface::registerFunctions()
{
	...
	lua_register(m_luaState, "sendChangeInvisible", LuaInterface::luaSendChangeInvisible);
...
int32_t LuaInterface::luaDoPlayerChangeVisible(lua_State* L)
{
	...
	if (Creature * creature = env->getCreatureByUID(popNumber(L)))
	{
		...
		{
			player->sendTextMessage(MSG_INFO_DESCR, "Voc  voltou a ficar vis vel.");
		...
		else if ((condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_GAMEMASTER, -1, 0, false, GAMEMASTER_INVISIBLE)))
		{
			...
			player->sendTextMessage(MSG_INFO_DESCR, "Voc  est  invis vel agora.");
		}
...
int32_t LuaInterface::luaSendChangeInvisible(lua_State* L)
{
    //sendChangeInvisible(cid)
    ScriptEnviroment* env = getEnv();
