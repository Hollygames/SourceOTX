...
void PrivateChatChannel::invitePlayer(Player* player, Player* invitePlayer)
{
	...
	msg << player->getName() << " convida você para seu canal de chat privado.";
	invitePlayer->sendTextMessage(MSG_INFO_DESCR, msg.str().c_str());

	msg.str("");
	msg << invitePlayer->getName() << " foi convidado.";
...
bool Chat::parseChannelNode(xmlNodePtr p)
{
	...
	std::string conditionMessage = "Você está mutado.";
	...
	switch(channelId)
	{
		...
		case CHANNEL_PRIVATE:
		{
			...
			for(uint16_t i = 100; i < 10000; ++i)
			{
				...
				if((newChannel = new PrivateChatChannel(i, player->getName() + " Cast", flags)))
				{
	...
	uint32_t statementId, bool anonymous/* = false*/)
{
	...
	if(!player->hasFlag(PlayerFlag_CannotBeMuted))
	{
		...
		{
			player->sendTextMessage(MSG_STATUS_SMALL, "Você não pode falar neste canal.");
			return true;
		}
		...
		{
			char buffer[100];
			sprintf(buffer, "Você não pode falar neste canal enquanto estiver no nível %d.", channel->getLevel());
	...
	{
		player->sendCancel("Você não está em uma guilda.");
		return true;
	}

	if(!IOGuild::getInstance()->guildExists(player->getGuildId()))
	{
		player->sendCancel("Parece que sua guilda não existe mais.");
		return true;
	}
	...
	if(text.substr(1) == "disband")
	{
		if(player->getGuildLevel() == GUILDLEVEL_LEADER)
		{
			...
			channel->talk("", MSG_GAMEMASTER_CHANNEL, "A guilda foi dissolvida.");
		}
		else
			player->sendCancel("Você não é o líder da sua guilda.");
	}
			...
			if(text.length() > 7)
			{
				...
				if(g_game.getPlayerByNameWildcard(param, paramPlayer) == RET_NOERROR)
				{
					if(paramPlayer->getGuildId() == 0)
					{
						...
						{
							sprintf(buffer, "%s convidou você para se juntar à guilda, %s. Você pode se juntar a esta guilda escrevendo: !joinguild %s", player->getName().c_str(), player->getGuildName().c_str(), player->getGuildName().c_str());
							paramPlayer->sendTextMessage(MSG_EVENT_GUILD, buffer);

							sprintf(buffer, "%s convidou %s para a guilda.", player->getName().c_str(), paramPlayer->getName().c_str());
							channel->talk("", MSG_CHANNEL_HIGHLIGHT, buffer);
							paramPlayer->invitationsList.push_back(player->getGuildId());
						}
						else
							player->sendCancel("Um jogador com esse nome já foi convidado para sua guilda.");
					}
					else
						player->sendCancel("Um jogador com esse nome já está em uma guilda.");
				}
				else if(IOLoginData::getInstance()->playerExists(param))
				{
					...
					if(!IOGuild::getInstance()->hasGuild(guid))
					{
						if(!IOGuild::getInstance()->isInvited(player->getGuildId(), guid))
						{
							if(IOGuild::getInstance()->guildExists(player->getGuildId()))
							{
								...
								sprintf(buffer, "%s convidou %s para a guilda.", player->getName().c_str(), param.c_str());
								channel->talk("", MSG_CHANNEL_HIGHLIGHT, buffer);
							}
							else
								player->sendCancel("Sua guilda não existe mais.");
						}
						else
							player->sendCancel("Um jogador com esse nome já foi convidado para sua guilda.");
					}
					...
					player->sendCancel("Um jogador com esse nome não existe.");
			}
			else
				player->sendCancel("Parâmetros de comando de guilda inválidos.");
		}
		else
			player->sendCancel("Você não tem direitos para convidar jogadores para sua guilda.");
	}
	else if(text.substr(1, 5) == "leave")
	{
		if(player->getGuildLevel() < GUILDLEVEL_LEADER)
		{
			if(!player->hasEnemy())
			{
				sprintf(buffer, "%s deixou a guilda.", player->getName().c_str());
				channel->talk("", MSG_CHANNEL_HIGHLIGHT, buffer);
				player->leaveGuild();
			}
			else
				player->sendCancel("Sua guilda está atualmente em guerra, você não pode deixá-la agora.");
		}
		else
			player->sendCancel("Você não pode deixar sua guilda porque você é o líder dela, você tem que passar a liderança para outro membro da sua guilda ou dissolver a guilda.");
	}
			...
			if(text.length() > 7)
			{
				...
				if(g_game.getPlayerByNameWildcard(param, paramPlayer) == RET_NOERROR)
				{
					if(paramPlayer->getGuildId() == 0)
					{
						...
						{
							sprintf(buffer, "%s revogou seu convite para a guilda %s.", player->getName().c_str(), (player->getSex(false) ? "his" : "her"));
							paramPlayer->sendTextMessage(MSG_EVENT_GUILD, buffer);

							sprintf(buffer, "%s revogou o convite da guilda de %s.", player->getName().c_str(), paramPlayer->getName().c_str());
							channel->talk("", MSG_CHANNEL_HIGHLIGHT, buffer);
							...
							player->sendCancel("Um jogador com esse nome não é convidado para sua guilda.");
					}
					else
						player->sendCancel("Um jogador com esse nome já está em uma guilda.");
				}
				else if(IOLoginData::getInstance()->playerExists(param))
				{
					...
					if(IOGuild::getInstance()->isInvited(player->getGuildId(), guid))
					{
						if(IOGuild::getInstance()->guildExists(player->getGuildId()))
						{
							sprintf(buffer, "%s revogou o convite de guilda de %s.", player->getName().c_str(), param.c_str());
							channel->talk("", MSG_CHANNEL_HIGHLIGHT, buffer);
