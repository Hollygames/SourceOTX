...
uint64_t IOLoginData::createAccount(std::string name, std::string password, uint32_t clientIp)
{
	...
	query << "INSERT INTO `accounts` (`id`, `name`, `password`, `lastDay`, `create_date`, `create_ip`) VALUES (NULL, " << db->escapeString(name) << ", " << db->escapeString(password) << ", " << timeNow << ", " << timeNow << ", " << clientIp <<")";
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	query.str("");
	query << "SELECT `pid`, `sid`, `itemtype`, `count`, `item_name`, `attributes` FROM `player_items` WHERE `player_id` = " << player->getGUID() << " ORDER BY `sid` DESC";
	if((result = db->storeQuery(query.str())))
	{
	...
	//load depot items
	query.str("");
	query << "SELECT `pid`, `sid`, `itemtype`, `count`, `item_name`, `attributes` FROM `player_depotitems` WHERE `player_id` = " << player->getGUID() << " ORDER BY `sid` DESC";
	if((result = db->storeQuery(query.str())))
	{
		loadItems(itemMap, result);
		for(ItemMap::reverse_iterator rit = itemMap.rbegin(); rit != itemMap.rend(); ++rit)
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	stmt.setQuery("INSERT INTO `player_items` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `item_name`, `attributes`) VALUES ");
	...
	{*/
		...
		if(itemList.size())
		{
			stmt.setQuery("INSERT INTO `player_depotitems` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `item_name`, `attributes`) VALUES ");
			if(!saveItems(player, itemList, stmt))
				return false;
...
{
    // Definição da função para substituir caracteres inválidos por espaços em branco no nome do item
    auto replaceInvalidCharacters = [](const std::string& str) {
        std::string result;
        for (char c : str) {
            // Verifica se o caractere é válido (dentro do intervalo ASCII)
            if (c >= 0 && c <= 127) {
                result += c;
            } else {
    ...
    for (ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it, ++runningId)
    {
        ...
        buffer << player->getGUID() << "," << it->first << "," << runningId << "," << item->getID() << ","
               << (int32_t)item->getSubType() << "," << db->escapeString(replaceInvalidCharacters(item->getName())) << ","  // Adiciona o nome do item
    ...
    while (stackList.size())
    {
        ...
        for (uint32_t i = 0; i < container->size(); ++i, ++runningId)
        {
            ...
            buffer << player->getGUID() << ","
                   ...
                   << db->escapeString(replaceInvalidCharacters(item->getName())) << ","  // Adiciona o nome do item
                   << db->escapeBlob(attributes, attributesSize);
