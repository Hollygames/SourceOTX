...
bool Player::challengeCreature(Creature* creature)
{
if(attackedCreature) {
    sendCancelTarget();
}
...
sendCreatureSquare(creature, 112);
sendMagicEffect(getPosition(), 13);
sendMagicEffect(creature->getPosition(), 14);
return true;
}
...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	if(guildId)
	{
		...
		if(lookDistance == -1)
			...
			s << " " << (sex % 2 ? "Ele" : "Ela") << " é ";
		s << (rankName.empty() ? "um membro" : rankName)<< " do " << guildName;
		if(!guildNick.empty())
			s << " (" << guildNick << ")";
...
int32_t Player::getCriticalHitChance() const
{
	...
	crit += std::max<int32_t>(0, getSkill(SKILL_FISH, SKILL_LEVEL)/ 2.85);
...
void Player::addSkillAdvance(skills_t skill, uint64_t count, bool useMultiplier/* = true*/)
{
    ...
    while (count > 0)  
    {
        uint64_t nextReqTries = vocation->getReqSkillTries(skill, skills[skill][SKILL_LEVEL]);

        uint64_t triesToAdd = std::min(count, nextReqTries - skills[skill][SKILL_TRIES]);

        count -= triesToAdd;
        skills[skill][SKILL_TRIES] += triesToAdd;

        // Atualize o percentual
        ...
        if (skills[skill][SKILL_TRIES] >= nextReqTries)
        {
            ...
            s.str("");
            s << "Você avançou para " << getSkillName(skill) << " level " << skills[skill][SKILL_LEVEL] << ".";
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		case RET_DESTINATIONOUTOFREACH:
			sendCancel("O destino esta fora de alcance.");
			break;

		case RET_NOTMOVABLE:
			sendCancel("Você nao pode mover este objeto.");
			break;

		case RET_DROPTWOHANDEDITEM:
			sendCancel("Solte o objeto de duas maos primeiro.");
			break;
		...
		case RET_CANNOTBEDRESSED:
			sendCancel("Você nao pode vestir este objeto la.");
			break;

		case RET_PUTTHISOBJECTINYOURHAND:
			sendCancel("Coloque esse objeto na sua mao.");
			break;

		case RET_PUTTHISOBJECTINBOTHHANDS:
			sendCancel("Coloque este objeto nas duas maos.");
			break;
		...
		case RET_NOTENOUGHCAPACITY:
			sendCancel("Este objeto e muito pesado para Você carregar.");
			break;

		case RET_CONTAINERNOTENOUGHROOM:
			sendCancel("Você nao pode colocar mais objetos neste conteiner.");
			break;
		...
		case RET_CANNOTPICKUP:
			sendCancel("Você nao pode pegar este objeto.");
			break;

		case RET_CANNOTTHROW:
			sendCancel("Você nao pode jogar la.");
			break;
		...
		case RET_THISISIMPOSSIBLE:
			sendCancel("Isto e impossivel.");
			break;

		case RET_PLAYERISPZLOCKED:
			sendCancel("Você nao pode entrar em uma zona de protecao depois de atacar outro jogador.");
			break;

		case RET_PLAYERISNOTINVITED:
			sendCancel("Você nao esta convidado.");
			break;

		case RET_CREATUREDOESNOTEXIST:
			sendCancel("Criatura nao existe.");
			break;

		case RET_DEPOTISFULL:
			sendCancel("Seu deposito esta cheio. Remova os itens excedentes antes de armazenar novos.");
			break;

		case RET_CANNOTUSETHISOBJECT:
			sendCancel("Você nao pode usar este objeto.");
			break;

		case RET_PLAYERWITHTHISNAMEISNOTONLINE:
			sendCancel("Um jogador com este nome nao esta online.");
			break;

		case RET_NOTREQUIREDLEVELTOUSERUNE:
			sendCancel("Você nao tem o nivel magico necessario para usar esta runa.");
			break;

		case RET_YOUAREALREADYTRADING:
			sendCancel("Você ja esta negociando. Termine este comercio primeiro.");
			break;

		case RET_THISPLAYERISALREADYTRADING:
			sendCancel("Essa pessoa ja esta negociando.");
			break;

		case RET_YOUMAYNOTLOGOUTDURINGAFIGHT:
			sendCancel("Você nao pode sair durante ou imediatamente apos uma luta.");
			break;

		case RET_DIRECTPLAYERSHOOT:
			sendCancel("Você nao tem permissão para atirar diretamente nos jogadores.");
			break;

		case RET_NOTENOUGHLEVEL:
			sendCancel("Você nao tem nível suficiente.");
			break;

		case RET_NOTENOUGHMAGICLEVEL:
			sendCancel("Você nao tem nível de magia suficiente.");
			break;

		case RET_NOTENOUGHMANA:
			sendCancel("Você não tem mana suficiente.");
			break;
		...
		case RET_YOUAREEXHAUSTED:
			sendCancel("Você está exausto.");
			break;

		case RET_CANONLYUSETHISRUNEONCREATURES:
			sendCancel("Você só pode usar esta runa em criaturas.");
			break;

		case RET_PLAYERISNOTREACHABLE:
			sendCancel("O jogador nao está acessivel.");
			break;

		case RET_CREATUREISNOTREACHABLE:
			sendCancel("A Criatura não está acessivel.");
			break;

		case RET_ACTIONNOTPERMITTEDINPROTECTIONZONE:
			sendCancel("Esta ação não e permitida em uma zona de protecao.");
			break;
		...
		case RET_YOUMAYNOTATTACKAPERSONINPROTECTIONZONE:
			sendCancel("Você não pode atacar uma pessoa em uma zona de protecao.");
			break;

		case RET_YOUMAYNOTATTACKAPERSONWHILEINPROTECTIONZONE:
			sendCancel("Você não pode atacar uma pessoa enquanto estiver em uma zona de protecao.");
			break;

		case RET_YOUCANONLYUSEITONCREATURES:
			sendCancel("Você so pode usa-lo em criaturas.");
			break;

		case RET_TURNSECUREMODETOATTACKUNMARKEDPLAYERS:
			sendCancel("Desative o modo seguro se Você realmente quiser atacar jogadores não marcados.");
			break;

		case RET_YOUNEEDPREMIUMACCOUNT:
			sendCancel("Você precisa de uma conta premium.");
			break;

		case RET_YOUNEEDTOLEARNTHISSPELL:
			sendCancel("Você precisa aprender esse feitiço primeiro.");
			break;

		case RET_YOURVOCATIONCANNOTUSETHISSPELL:
			sendCancel("Sua vocacao não pode usar este feitico.");
			break;

		case RET_YOUNEEDAWEAPONTOUSETHISSPELL:
			sendCancel("Você precisa equipar uma arma para usar esse feitico.");
			break;

		case RET_PLAYERISPZLOCKEDLEAVEPVPZONE:
			sendCancel("Você não pode sair de uma zona pvp depois de atacar outro jogador.");
			break;

		case RET_PLAYERISPZLOCKEDENTERPVPZONE:
			sendCancel("Você não pode entrar em uma zona pvp depois de atacar outro jogador.");
			break;

		case RET_ACTIONNOTPERMITTEDINANOPVPZONE:
			sendCancel("Esta ação não e permitida em uma zona segura.");
			break;

		case RET_YOUCANNOTLOGOUTHERE:
			sendCancel("Você não pode sair aqui.");
			break;

		case RET_YOUNEEDAMAGICITEMTOCASTSPELL:
			sendCancel("Você precisa de um item mágico para lançar este feitico.");
			break;

		case RET_CANNOTCONJUREITEMHERE:
			sendCancel("Você nao pode conjurar itens aqui.");
			break;

		case RET_NAMEISTOOAMBIGUOUS:
			sendCancel("O nome é muito ambiguo.");
			break;
		...
		case RET_YOUARENOTTHEOWNER:
			sendCancel("Você não é o dono.");
			break;

		case RET_YOUMAYNOTCASTAREAONBLACKSKULL:
			sendCancel("Você nao pode lançar feitiços de area enquanto tiver uma caveira preta.");
			break;

		case RET_TILEISFULL:
			sendCancel("Você não pode adicionar mais itens neste bloco.");
			break;

		case RET_NOTENOUGHSKILL:
			sendCancel("Você não tem habilidade suficiente.");
			break;
		...
		case RET_YOUMAYNOTATTACKIMMEDIATELYAFTERLOGGINGIN:
			sendCancel("Você nao pode atacar imediatamente apos o login.");
			break;

		case RET_YOUCANONLYTRADEUPTOX:
		{
			std::stringstream s;
			s << "Você so pode negociar ate " << g_config.getNumber(ConfigManager::TRADE_LIMIT) << " itens por vez.";
			sendCancel(s.str());
		}
			break;
...
void Player::sendCreatureChangeVisible(const Creature* creature, Visible_t visible)
{
	...
	const Player* player = creature->getPlayer();
	if(player == this || (player && (visible < VISIBLE_GHOST_APPEAR))
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	if(g_config.getBool(ConfigManager::DISPLAY_LOGGING))
		std::clog << name << " entrou no jogo." << std::endl;
}
...
void Player::onCreatureDisappear(const Creature* creature, bool isLogout)
{
	...
	if(g_config.getBool(ConfigManager::DISPLAY_LOGGING))
		std::clog << getName() << " foi desconectado." << std::endl;
...
void Player::removeMessageBuffer()
{
	...
	char buffer[50];
	sprintf(buffer, "Você está mudo por %d segundos", muteTime);
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier/* = true*/)
{
	...
	while(manaSpent + amount >= nextReqMana)
	{
		...
		s << "Você avançou para o Ninjutsu " << ++magLevel << ".";
...
void Player::addExperience(uint64_t exp)
{
	...
	if(prevLevel != level)
	{
		...
		s << "Você avançou para o level " << level << ".";
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	if(prevLevel != level)
	{
		...
		std::stringstream s;
		s << "Você avançou do Level " << prevLevel << " para o Level " << level << ".";
	...
	bool checkDefense/* = false*/, bool checkArmor/* = false*/, bool reflect/* = true*/, bool field/* = false*/, bool element/* = false*/)
{
	...
	if(damage > 0) { 
		uint32_t chance = getKawaramiChance();
		uint32_t random = random_range(1, 100);
		if(chance >= random)
		{
			damage = 75;
...
Item* Player::createCorpse(DeathList deathList)
{
	...
	std::stringstream ss;
	ss << "Você vê " << nameDescription << ". " << (sex % 2 ? "Ele foi morto" : "Ele foi morto") << " por ";
	if(deathList[0].isCreatureKill())
	{
		...
		if(deathList[0].getKillerCreature()->getMaster())
			ss << " summonado por " << deathList[0].getKillerCreature()->getMaster()->getNameDescription();
	}
	else
	...
	if(deathList.size() > 1 && g_config.getBool(ConfigManager::MULTIPLE_NAME))
	{
		if(deathList[0].getKillerType() != deathList[1].getKillerType())
		{
			...
			{
				ss << " e por " << deathList[1].getKillerCreature()->getNameDescription();
				if(deathList[1].getKillerCreature()->getMaster())
					ss << " summonado por " << deathList[1].getKillerCreature()->getMaster()->getNameDescription();
			}
			else
				ss << " e por " << deathList[1].getKillerName();
		}
...
bool Player::addVIP(uint32_t _guid, const std::string& name, bool online, bool loading/* = false*/)
{
	...
	{
		if(!loading)
			sendTextMessage(MSG_STATUS_SMALL, "Você nï¿½o pode adicionar Você mesmo.");
	...
	{
		sendTextMessage(MSG_STATUS_SMALL, "Você nï¿½o pode adicionar mais amigos.");
		return false;
	}
	...
	{
		if(!loading)
			sendTextMessage(MSG_STATUS_SMALL, "Este jogador jï¿½ está na sua lista.");
...
void Player::onAddCombatCondition(ConditionType_t type, bool)
{
	...
	switch(type)
	{
		//client hardcoded
		case CONDITION_FIRE:
			sendTextMessage(MSG_STATUS_DEFAULT, "Você está pegando fogo.");
			break;
		case CONDITION_POISON:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está envenenado.");
			break;
		case CONDITION_ENERGY:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está eletrocultado.");
			break;
		case CONDITION_FREEZING:
			sendTextMessage(MSG_STATUS_DEFAULT, "Você está congelando.");
			break;
		case CONDITION_DAZZLED:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está deslumbrado.");
			break;
		case CONDITION_CURSED:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está amaldiçoado.");
			break;
		case CONDITION_DROWN:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está se afogando.");
			break;
		case CONDITION_DRUNK:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está bêbado.");
			break;
		case CONDITION_PARALYZE:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está paralisado.");
			break;
		case CONDITION_BLEEDING:
		sendTextMessage(MSG_STATUS_DEFAULT, "Você está sangrando.");
			break;
...
bool Player::addUnjustifiedKill(const Player* attacked, bool countNow)
{
	...
	if(countNow)
	{
		...
		sprintf(buffer, "Atenção! O assassinato de %s não foi justificado.", attacked->getName().c_str());
...
void Player::increaseCombatValues(int32_t& min, int32_t& max, bool useCharges, bool countWeapon)
{
	...
	if(max > 0) {
		...
		std::string vocation_name = vocation->getName();
	std::transform(vocation_name.begin(), vocation_name.end(), vocation_name.begin(), ::tolower);
	if (min < 0 && max < 0 && vocation_name.find("kisame") != std::string::npos) {
		...
		for (int8_t x = -3; x <= 3; x++) {
			for (int8_t y = -3; y <= 3; y++) {
				...
				if (Tile* tile = g_game.getMap()->getTile(pos)) {
					if (tile->ground &&  tile->ground->getName().find("water") != std::string::npos) {
						findWater = true;
						break;
						break;
					}
...
void Player::sendCritical() const
{
	...
	if(g_config.getBool(ConfigManager::DISPLAY_CRITICAL_HIT))
		g_game.addAnimatedText(getPosition(), COLOR_RED, "CRITICAL!");
}
...
if(!hasPoints)
  return false;
    ...
    if ((result = db->storeQuery(query.str())))
    {
        ...
        // Verifique a sigla no nome do item
        int raridadeLevel = 0;
        if (item_name.find("[N]") != std::string::npos) raridadeLevel = 1;
        else if (item_name.find("[R]") != std::string::npos) raridadeLevel = 2;
        else if (item_name.find("[L]") != std::string::npos) raridadeLevel = 3;

        if (item_seller == getName())
        {
            sendFYIBox("You cannot buy your own offering.");
            return false;
        }
        ...
        if ((int)countSelectedScrollBar > result->getDataInt("amount")) {
            sendFYIBox("You cannot buy more than what is on offer.");
            return false;
        }
        ...
        if(Item* item = Item::CreateItem(item_id, countSelectedScrollBar))
        {
            ...
            // Configuração de atributos e nome com base na raridade
            if (raridadeLevel >= 1 && raridadeLevel <= 3)
            {
                item->setAttribute("magiclevel", raridadeLevel);
                item->setAttribute("skillfist", raridadeLevel);
            ...
            ReturnValue ret = g_game.internalMoveItem(NULL, item->getParent(), this, INDEX_WHEREEVER, item, countSelectedScrollBar, 0);
            if (ret != RET_NOERROR) {
                return false;
            }
			
			sendFYIBox("You have successfully purchased the item.");
		...
		sendFYIBox("The offer you are trying to purchase is no longer available.");
		return false;
	}
	return true;
}
...
bool Player::confirmMarketOfferToPlayer(uint64_t transaction_id, std::string item_seller, uint16_t index) // Index is not in using!!
{
	...
	setStorage(storage_str.str().c_str(), "offerted");
	return true;
}
...
bool Player::sendMarketCancelYourOffer(uint64_t transaction_id)
{
	Database* db = Database::getInstance();
	DBQuery query;
	DBResult_ptr result;
	...
	query.str("");
	query << "SELECT `transaction_id`, `item_id2`, `amount`, `description1`, `attributes` FROM `players_mymarketmakeoffforyour` WHERE `transaction_id` = " << transaction_id << ";";
	if ((result = db->storeQuery(query.str())))
	{
		do {
			uint16_t item_id = result->getDataInt("item_id2");
	...
	setStorage(storage_str.str().c_str(), "none");
	openMarketOfferYourOffers();
	return true;
}
...
bool Player::sendMarketChangePage(std::string type, std::string category, uint64_t page)
{
	...
	uint16_t row_count = 0;
	query.str("");
	query << "SELECT `item_id`, `item_name`, `item_seller`, `amount`, `price`, `gender`, `level`, `ispokemon`, `attributes`, `description`, `id`, `item_time`, `transaction_id`, `onlyoffers` FROM `players_marketoffers` WHERE `page_numeration` = " << page_numeration << " ORDER BY `page_numeration` ASC;";
	if ((result = db->storeQuery(query.str()))) {
		do {
			uint16_t item_id = result->getDataInt("item_id");
			uint16_t amount = result->getDataInt("amount");
...
bool Player::addOfflineTrainingTries(skills_t skill, int32_t tries)
{
	...
	if(skill == SKILL__MAGLEVEL)
	{
		...
		ss << "Você avançou para o Ninjutsu " << magLevel << ".";
	...
	else
	{
		...
		ss << "Você avançou para o level " << level << ".";
		sendTextMessage(MSG_EVENT_ADVANCE, ss.str().c_str());
		ss.str("");
	}
...
uint32_t Player::getKawaramiChance() const
{
	uint32_t chance = std::max<uint32_t>(0, 1 + (getSkill(SKILL_SHIELD, SKILL_LEVEL) / 15 ));
	return chance;
}