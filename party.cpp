...
void Party::disband()
{
	...
	leader->sendTextMessage(MSG_INFO_DESCR, "Seu grupo foi desfeito.");
	...
	for(PlayerVector::iterator it = memberList.begin(); it != memberList.end(); ++it)
	{
		...
		(*it)->sendTextMessage(MSG_INFO_DESCR, "Seu grupo foi desfeito.");
...
bool Party::leave(Player* player)
{
	...
	player->sendTextMessage(MSG_INFO_DESCR, "Você saiu do grupo.");
	...
	sprintf(buffer, "%s saiu do grupo.", player->getName().c_str());
...
bool Party::passLeadership(Player* player)
{
	...
	sprintf(buffer, "%s é o lider do grupo agora.", player->getName().c_str());
	broadcastMessage(MSG_INFO_DESCR, buffer, true);
	
	player->sendTextMessage(MSG_INFO_DESCR, "Você agora é o líder do grupo.");
	updateSharedExperience();
...
bool Party::join(Player* player)
{
	...
	char buffer[200];
	sprintf(buffer, "%s entrou no grupo.", player->getName().c_str());
	broadcastMessage(MSG_PARTY, buffer);

	sprintf(buffer, "Você entrou no grupo de %s. Abra o canal do grupo para se comunicar com seus companheiros.", leader->getName().c_str());
...
void Party::revokeInvitation(Player* player)
{
	...
	sprintf(buffer, "%s revogou %s o convite.", leader->getName().c_str(), (leader->getSex(false) ? "sua" : "seu"));
	player->sendTextMessage(MSG_INFO_DESCR, buffer);

	sprintf(buffer, "Convite para %s foi revogado.", player->getName().c_str());
...
bool Party::invitePlayer(Player* player)
{
	...
	sprintf(buffer, "%s foi convidado. Abra o canal do grupo para se comunicar com seus membros.", player->getName().c_str());
	leader->sendTextMessage(MSG_INFO_DESCR, buffer);

	sprintf(buffer, "%s convidou você para a grupo.", leader->getName().c_str());
	player->sendTextMessage(MSG_INFO_DESCR, buffer);
...
bool Party::setSharedExperience(Player* player, bool _sharedExpActive)
{
	...
	if(sharedExpActive)
	{
		...
		if(sharedExpEnabled)
			leader->sendTextMessage(MSG_INFO_DESCR, "A experiência compartilhada agora está ativa.");
		else
			leader->sendTextMessage(MSG_INFO_DESCR, "A experiência compartilhada foi ativada, mas alguns membros da sua parte estão inativos.");
	}
	else
		leader->sendTextMessage(MSG_INFO_DESCR, "A experiÃªncia compartilhada foi desativada.");
